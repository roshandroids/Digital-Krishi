...
class AddNewsPortal extends StatefulWidget {
  @override
  _AddNewsPortalState createState() => _AddNewsPortalState();
}

class _AddNewsPortalState extends State<AddNewsPortal> {
  bool _isLoading = false;
  final picker = ImagePicker();

  TextEditingController portalname = TextEditingController();
  TextEditingController portalUrl = TextEditingController();
  ...
  void _post(BuildContext context) async {
    if (portalname.text.isNotEmpty &&
        portalUrl.text.isNotEmpty &&
        _image != null) {
      setState(() {
        _errorMessage = "";
        _isLoading = true;
      ...
      try {
        ...
        await databaseReference.collection('newsPortal').document().setData(
          {
            'url': portalUrl.text.trim(),
            'siteName': portalname.text.trim(),
            'siteLogo': imgUrl,
          },
        );
        setState(() {
                ...
                child: Container(
                  ...
                  child: Column(
                    children: <Widget>[
                      SizedBox(height: 10),
                      TextFormField(
                        controller: portalname,
                        onSaved: (postTitle) => postTitle = postTitle.trim(),
                        decoration: InputDecoration(
                          labelText: "News Portal Name",
                          hintText: "News Portal Name",
                          border: OutlineInputBorder(),
                        ),
                      ),
                      SizedBox(height: 15),
                      TextFormField(
                        controller: portalUrl,
